...
using VideoScan.Migrations.SeedData;
...
namespace VideoScan.Tests
{
    public abstract class VideoScanTestBase : AbpIntegratedTestBase<VideoScanTestModule>
    {
        ...
        protected void UsingDbContext(Action<VideoScanDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<VideoScanDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<VideoScanDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<VideoScanDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<VideoScanDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<VideoScanDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<VideoScanDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<VideoScanDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<VideoScanDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<VideoScanDbContext, Task<T>> func)
        {
            T result;
